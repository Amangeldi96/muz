...
window.togglePlay = function(btn, src) {
    ...
    // Эгер басылган ошол эле видео болсо
    if (currentBtn === btn) {
        const state = ytPlayer.getPlayerState();
        state === YT.PlayerState.PLAYING ? ytPlayer.pauseVideo() : ytPlayer.playVideo();
        btn.innerHTML = (state === YT.PlayerState.PLAYING ? iconHTML : pauseIconHTML);
        return;
    }
...
window.onYouTubeIframeAPIReady = function() {
    console.log("YouTube API даяр болду!");
...
function onPlayerStateChange(event) {
    ...
    if (state === YT.PlayerState.PLAYING) currentBtn.innerHTML = pauseIconHTML;
    else if (state === YT.PlayerState.PAUSED || state === YT.PlayerState.ENDED) currentBtn.innerHTML = iconHTML;
}
